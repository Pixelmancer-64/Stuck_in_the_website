...
export function easyLoop(times, callback) {
...
export class Point extends Particle {
  ...
  fill(cellSize = 1, ctx) {
    ...
    ctx.arc(x * cellSize, y * cellSize, this.radius, 0, Math.PI * 2);
    ctx.fill();
  }

  stroke(cellSize = 1, ctx) {
    const { x, y } = this.pos;
    ctx.beginPath();
...
export class Square extends Particle {
  ...
  fill(cellSize = 1, ctx) {
    ...
    ctx.beginPath();
    ctx.fillStyle = this.color;
    ctx.fillRect(x * cellSize, y * cellSize, this.width, this.height);
  }
  ...
  stroke(cellSize = 1, ctx) {
    ...
    ctx.beginPath();
    ctx.strokeStyle = this.color;
    ctx.rect(x * cellSize, y * cellSize, this.width, this.height);
    ctx.stroke();
  }
}
